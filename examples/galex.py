...
rc("text", usetex=True)
import daft
pgm = daft.PGM([5.4, 6.6], origin=[1.2, 1.1])
wide = 1.5
verywide = 1.5 * wide

# electrons
el_x, el_y = 2., 2.
pgm.add_plate(daft.Plate([el_x - 0.6, el_y - 0.7, 2.2, 3.2], label="electrons $i$"))
pgm.add_node(daft.Node("raw6", r"raw6$_i$", el_x, el_y, aspect=wide, observed=True))
pgm.add_node(daft.Node("xabc", r"xa$_i$,xabc$_i$,ya$_i$,\textit{etc}", el_x + 0.5, el_y + 1, aspect=2.3 * wide))
pgm.add_edge("xabc", "raw6")
pgm.add_node(daft.Node("xyti", r"$x_i,y_i,t_i$", el_x + 1., el_y + 2, aspect=wide))
...
# s/c
sc_x, sc_y = ph_x + 1.5, ph_y - 2.
pgm.add_node(daft.Node("dark", r"dark", sc_x, sc_y - 1, aspect=wide))
pgm.add_edge("dark", "xyti")
pgm.add_node(daft.Node("flat", r"flat", sc_x, sc_y, aspect=wide))
pgm.add_edge("flat", "xyti")
pgm.add_node(daft.Node("att", r"att", sc_x, sc_y + 3))
pgm.add_edge("att", "Ixnt")
pgm.add_node(daft.Node("optics", r"optics", sc_x, sc_y + 2, aspect=wide))
pgm.add_edge("optics", "Ixyt")
pgm.add_node(daft.Node("psf", r"psf", sc_x, sc_y + 1))
pgm.add_edge("psf", "xyti")

# sky
pgm.add_node(daft.Node("sky", r"sky", sc_x, sc_y + 4))
pgm.add_edge("sky", "Iadt")

# stars
star_x, star_y = 2., 6.
pgm.add_plate(daft.Plate([star_x - 0.6, star_y - 0.7, 2.2, 2.2], label="stars $n$"))
pgm.add_node(daft.Node("star adt", r"$I_{\nu,n}(\alpha,\delta,t)$", star_x + 0.5, star_y + 1, aspect=verywide))
